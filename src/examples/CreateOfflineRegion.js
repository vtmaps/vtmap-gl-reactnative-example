...
class CreateOfflineRegion extends React.Component {
  ...
  componentWillUnmount() {
    ...
    VTMapGL.offlineManager.deletePack(this.state.name);
    VTMapGL.offlineManager.unsubscribe('test');
  }
  ...
  async onDidFinishLoadingStyle() {
    ...
    // start download
    VTMapGL.offlineManager.createPack(options, this.onDownloadProgress);
  }
  ...
  _getRegionDownloadState(downloadState) {
    switch (downloadState) {
      case VTMapGL.OfflinePackDownloadState.Active:
        return 'Active';
      case VTMapGL.OfflinePackDownloadState.Complete:
        return 'Complete';
      default:
        return 'Inactive';
    }
    ...
    return (
      <Page {...this.props}>
        <VTMapGL.MapView
          ...
          <VTMapGL.Camera zoomLevel={10} centerCoordinate={CENTER_COORD} />
        </VTMapGL.MapView>
